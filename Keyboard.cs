...
namespace Starfury
{
    ...
    {
        // From https://github.com/opentk/opentk/blob/d50c2fcea0696d3cc82aeb339e15c049405129ba/src/OpenTK/Input/Key.cs
        ...
        public static Dictionary<OpenTK.Input.Key, uint> Keycode = new Dictionary<OpenTK.Input.Key, uint>();

        public static void Initialize()
        {
            foreach (uint i in Enumerable.Range(0, KeyMap.Length))
            {
                if (!Keycode.ContainsKey(KeyMap[i]))
                {
                    Keycode[KeyMap[i]] = i;
                }
            }
            Console.WriteLine("Test loop Keycode[" + Key.BackSpace + "] = " + Keycode[Key.BackSpace]);
        }
    }
}